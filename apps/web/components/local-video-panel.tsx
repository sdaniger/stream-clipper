...
export function LocalVideoPanel({ candidates, onClipGenerated, onTranscriptionComplete }: LocalVideoPanelProps) {
  ...
  // Auto-populate inputPath from the selected candidate's sourceVideoPath.
  // This eliminates the need for users to manually type the video path when
  // candidates were generated by the archive pipeline or imported with video metadata.
  useEffect(() => {
    if (selectedCandidate?.sourceVideoPath) {
      setInputPath(selectedCandidate.sourceVideoPath);
    }
  }, [selectedCandidate?.sourceVideoPath]);
  ...
  async function refreshAvailableVideos() {
    try {
      const response = await fetch("/api/media/list-videos");
      const data = await response.json();
      if (response.ok && Array.isArray(data.videos)) {
        setAvailableVideos(data.videos);
      }
    } catch {
      // Ignore errors — available videos are a nice-to-have
    }
  }
          ...
          <div className="space-y-5">
            ...
            <div className="grid gap-4 lg:grid-cols-[1fr_auto]">
              <label className="block">
                ...
                />
                {availableVideos.length > 0 && !availableVideos.includes(inputPath) && (
                  <select
                    value=""
                    onChange={(event) => {
                      if (event.target.value) setInputPath(event.target.value);
                    }}
                    className="mt-2 h-10 w-full rounded-2xl border border-white/10 bg-white/[0.04] px-4 text-xs text-slate-300 outline-none focus:border-emerald-200/60"
                  >
                    <option value="">{t("media.selectAvailableVideo")}</option>
